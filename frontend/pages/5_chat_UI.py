...
async def chat_with_rag(prompt, _document_content):
    ...
    async with httpx.AsyncClient(cookies=st.session_state.httpx_cookies) as client:
        response = await client.post(url=f"{CHAT_URL}/chat/", json={"prompt": prompt})
        if response.status_code == 200:
            return response.json()  # Job done, return result
        else:
            logger.error(
                f"Error in chat_with_rag: {response.status_code} - {response.text}"
            )
...
with col1:
    if st.button("What is the main topic?"):
        response = asyncio.run(
            chat_with_rag("What is the main topic?", "document content")
        )
...
with col2:
    ...
    if st.button("What are the key findings?"):
        response = simulate_rag_response(
            "What are the key findings?", "document content"
        )
        st.session_state.messages.append(
