...
class QwenRAGConfig:
    ...
    def __init__(self):
        ...
        self.fallback_to_keyword_classification = os.getenv("FALLBACK_TO_KEYWORD_CLASSIFICATION", "true").lower() == "true"
...
class QwenRAGOptimizer:
    ...
    @staticmethod
    def fallback_keyword_classification(question: str) -> str:
        """Fallback keyword-based classification with enhanced patterns"""
        question_lower = question.lower().strip()
        
        # Enhanced keyword patterns with context awareness
        factual_patterns = [
            # Direct factual keywords
            'what is', 'when did', 'where is', 'who is', 'how many', 'how much',
            'define', 'definition of', 'meaning of',
            # List and enumeration requests
            'list', 'show me', 'display', 'enumerate', 'give me the',
            # Specific data requests
            'statistics', 'data', 'number of', 'percentage of', 'rate of',
            # Simple factual questions
            'is there', 'does', 'do', 'can you tell me', 'what are the'
        ]
        
        analytical_patterns = [
            # Analysis keywords
            'why', 'how does', 'analyze', 'analyse', 'compare', 'comparison',
            'evaluate', 'assessment', 'assess', 'examine', 'investigation',
            # Reasoning and relationship keywords
            'relationship', 'correlation', 'impact', 'effect', 'influence',
            'implications', 'consequences', 'pros and cons', 'advantages',
            'disadvantages', 'benefits', 'drawbacks', 'trade-offs',
            # Causal and explanatory
            'factors', 'reasons', 'causes', 'contributes to', 'leads to'
        ]
        
        summarization_patterns = [
            # Direct summarization requests
            'summarize', 'summarise', 'summary', 'overview', 'main points',
            'key findings', 'key points', 'highlights', 'takeaways',
            # Condensation requests
            'brief', 'concise', 'condensed', 'in short', 'gist',
            'executive summary', 'synopsis', 'abstract'
        ]
        
        # Check for factual patterns (but be careful with ambiguous cases)
        for pattern in factual_patterns:
            if pattern in question_lower:
                # Special handling for potentially ambiguous cases
                if any(summ_pattern in question_lower for summ_pattern in ['summarize', 'summary', 'overview']):
                    continue  # Let summarization take precedence
                return QueryType.FACTUAL.value
        
        # Check for summarization patterns
        for pattern in summarization_patterns:
            if pattern in question_lower:
                return QueryType.SUMMARIZATION.value
        
        # Check for analytical patterns
        for pattern in analytical_patterns:
            if pattern in question_lower:
                return QueryType.ANALYTICAL.value
    ...
    async def detect_query_type(
        ...
        """
        Detect query type using LLM classification with keyword fallback
        """
        # Try LLM-based classification first if enabled and client is available
        if (config and config.enable_llm_query_classification and 
            ...
            llm_classification = await QwenRAGOptimizer.classify_query_with_llm(
                question, openai_client, model_name, config.classification_params
            )
            
            # If LLM classification succeeded, return it
            if llm_classification != QueryType.GENERAL.value or not config.fallback_to_keyword_classification:
                return llm_classification
        
        # Fallback to keyword-based classification
        keyword_classification = QwenRAGOptimizer.fallback_keyword_classification(question)
        logger.info(f"Using fallback keyword classification: {keyword_classification}")
