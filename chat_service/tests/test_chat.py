...
class TestPrepareRetrievalResults:
    ...
    def test_valid_results(self, sample_chroma_results):
        ...
        assert 0 < chunks[0]['similarity_score'] <= 1  # L2 distance converted to similarity
...
class TestRerankChunks:
    ...
    async def test_multiple_document_chunks(self):
        ...
        assert reranked[0]['doc_id'] == 'doc1' and reranked[0]['similarity_score'] == 0.9
        assert reranked[1]['doc_id'] == 'doc2' and reranked[1]['similarity_score'] == 0.85
...
class TestPerformRAGQuery:
    ...
    async def test_no_chunks_found(self):
        ...
        with patch('routers.chat.get_chroma_client') as mock_chroma, \
    ...
    async def test_successful_rag_query(self, sample_chroma_results):
        ...
        with patch('routers.chat.get_chroma_client') as mock_chroma, \
            ...
            mock_optimizer.chunk_optimization.return_value = (mock_chunks, "test context")
...
class TestChatEndpoint:
    def test_invalid_query_validation(self, sample_chat_request):
        ...
        with patch('routers.chat.validate_query_with_llm') as mock_validate:
            ...
            test_client.app.dependency_overrides[get_openai_client] = lambda: mock_ai_client
    ...
    def test_successful_chat_response(self, sample_chat_request):
        ...
        with patch('routers.chat.validate_query_with_llm') as mock_validate, \
            ...
            mock_rag.return_value = (
                "user prompt", mock_chunks, "system prompt", "general"
            )
