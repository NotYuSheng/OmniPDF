...
class TestProcessPdf:
    ...
    def test_successful_pdf_processing(
        ...
        mock_result.document.iterate_items.return_value = [(mock_picture_item, None)]
    ...
    def test_s3_upload_failure(
        self, mock_save_job, mock_converter_class, mock_upload, sample_docling_data
    ):
...
class TestExtractorRoutes:
    def test_submit_pdf_endpoint(self, client):
        ...
        with (
            patch("routers.extractor.save_job") as mock_save_job,
            patch("routers.extractor.process_pdf"),
        ):
            response = client.post(
                "/documents/extract",
                params={"doc_id": "test_doc", "download_url": "http://test.com/pdf"},
            )
    ...
    def test_get_status_processing(self, client):
        ...
        with patch("routers.extractor.load_job") as mock_load_job:
            mock_load_job.return_value = {"status": "processing", "data": {}}
