...
class TestTranslateFunction:
    ...
    async def test_translate_timeout_retry(self, mock_client):
        ...
        mock_client_instance.post = AsyncMock(side_effect=[
            ...
            httpx.ReadTimeout("Timeout"),
            MagicMock(json=lambda: {"choices": [{"message": {"content": "Hello"}}]}, raise_for_status=MagicMock())
        ])
        mock_client.return_value.__aenter__.return_value = mock_client_instance
...
class TestSafeTranslateFunction:
    ...
    async def test_safe_translate_with_text(self, mock_translate):
        ...
        mock_translate.assert_called_once_with("Original text", source_lang="Spanish", target_lang="English")
...
class TestTranslationRouter:
    ...
    def test_doc_translate_success(self, mock_safe_translate, mock_upload, mock_save_job):
